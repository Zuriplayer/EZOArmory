...
$manifest = Join-Path $root "EZOArmory.txt"
...
$coreVersion = Get-RegexValue $coreText '^\s*EZOArmory\.ADDON_VERSION\s*=\s*"([^"]+)"\s*$'
$configVersion = Get-RegexValue $configText '"version":\s*"([^"]+)"'

if ($Check) {
    $ok = $true
    if ($manifestVersion -ne $coreVersion) {
        Write-Error "Version mismatch: EZOArmory.txt=$manifestVersion modules/core.lua=$coreVersion"
        $ok = $false
    }
    if ($manifestVersion -ne $configVersion) {
        Write-Error "Version mismatch: EZOArmory.txt=$manifestVersion ezo-addon.json=$configVersion"
        $ok = $false
    }
    if (-not $manifestAddOnVersion) {
        Write-Error "Missing ## AddOnVersion in EZOArmory.txt"
        $ok = $false
    }
        ...
        try {
            ...
            foreach ($expectedApiToken in $expectedApiTokens) {
                if ($manifestApiTokens -notcontains $expectedApiToken) {
                    Write-Error "APIVersion mismatch: EZOArmory.txt='$manifestApiVersion' does not include expected API '$expectedApiToken'"
                    $ok = $false
                }
            }
        } catch {
...
if ($manifestVersion -ne $coreVersion) {
    throw "Refusing to bump from inconsistent state: EZOArmory.txt=$manifestVersion modules/core.lua=$coreVersion"
}
if ($manifestVersion -ne $configVersion) {
    throw "Refusing to bump from inconsistent state: EZOArmory.txt=$manifestVersion ezo-addon.json=$configVersion"
}
...
$coreText = Set-RegexValue $coreText '^(\s*EZOArmory\.ADDON_VERSION\s*=\s*")[^"]+("\s*)$' $Version
Write-Text $core $coreText

$configText = Set-RegexValue $configText '^(\s*"version":\s*")[^"]+(",?\s*)$' $Version
$configText = Set-RegexValue $configText '^(\s*"zipName":\s*"EZOArmory_v)[^"]+(\.zip",?\s*)$' $Version
